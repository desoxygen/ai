...
from ai_scientist.loop_guard import StageGuard, StageSkip, RunAborted
from ai_scientist.research_quality import load_learnings
...
def generate_ideas(
    ...
    # Knowledge base (learnings.md): what was already tried and how it ended.
    # Fed to the model so new ideas stop repeating dead ends.
    learnings = load_learnings(osp.basename(base_dir.rstrip("/\\")))
    learnings_block = f"\n\nWhat has already been tried in this project (do NOT repeat these, learn from them):\n\n{learnings}\n" if learnings else ""

    guard = StageGuard("idea-generation")
    seen_names = set()
    ...
    for _ in range(max_num_generations):
        ...
        try:
            ...
            # Loop protection: the same idea Name regenerating over and over
            # means the model is going in circles.
            idea_name = json_output.get("Name", "")
            if idea_name and idea_name in seen_names:
                guard.check_repeat("duplicate-idea", idea_name)
            seen_names.add(idea_name)
            guard.success()
        except (StageSkip, RunAborted):
            raise
    ...
    out_path = osp.join(base_dir, "ideas.json")
    # never destroy the previous idea set without a backup — generation runs
    # overwrite this file, and it may hold days of generated ideas
    if osp.exists(out_path):
        try:
            stamp = time.strftime("%Y%m%d_%H%M%S")
            shutil.copy(out_path, osp.join(base_dir, f"ideas.backup-{stamp}.json"))
        except Exception:
            pass
...
def check_idea_novelty(
    ...
    for idx, idea in enumerate(ideas):
        ...
        msg_history = []
        papers_str = ""
        guard = StageGuard(f"novelty:{idea['Name']}")
        related = {}  # title -> paper info, collected from the actual searches
        ...
        for j in range(max_num_iterations):
            try:
                ...
                # Loop protection: searching the same query again and again
                # is a cycle — the guard will ask the user what to do.
                guard.check_repeat(f"query:{idea['Name']}", str(query))
                papers = search_for_papers(query, result_limit=10, engine=engine)
                if papers is None:
                    papers_str = "No papers found."
                else:
                    # keep the closest matches as real citations for the idea
                    for p in papers[:10]:
                        title = str(p.get("title", ""))[:200]
                        if title and title not in related:
                            related[title] = {
                                "title": title,
                                "venue": str(p.get("venue", ""))[:80],
                                "year": p.get("year"),
        ...
        idea["novel"] = novel
        if related:
            # real citations found during the search, closest first
            idea["related_work"] = sorted(
                related.values(),
