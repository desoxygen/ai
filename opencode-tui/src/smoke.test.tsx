...
test("new project wizard: enforces required info, creates prompt.json + seed_ideas.json", async () => {
  ...
  try {
    ...
    await frameUntil(setup, (f) => f.includes("new project — step 1/5"))
    ...
    setup.mockInput.pressEnter()
    const conf = await frameUntil(setup, (f) => f.includes("step 5/5"))
...
test("@ file autocomplete fuzzy-searches the project tree", async () => {
  ...
  try {
    ...
    const menu = await frameUntil(setup, (f) => f.includes("@src/App.tsx"), 8000)
